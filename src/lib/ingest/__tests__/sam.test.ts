...
function mapOpportunityToRecord(opp: {
  noticeId: string
  title?: string | null
  ...
  placeOfPerformance?: { state?: { name?: string } | null } | null
  ...
  test('Property 3: Ingestion upsert is idempotent', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
            noticeId: fc.string({ minLength: 1, maxLength: 20 }).map(s => `NOTICE-${s}`),
            title: fc.option(fc.string({ minLength: 1, maxLength: 100 }), { nilWithUndefined: true }),
            fullParentPathName: fc.option(fc.string({ maxLength: 100 }), { nilWithUndefined: true }),
            organizationName: fc.option(fc.string({ maxLength: 100 }), { nilWithUndefined: true }),
            naicsCode: fc.option(fc.string({ maxLength: 10 }), { nilWithUndefined: true }),
            type: fc.option(fc.string({ maxLength: 20 }), { nilWithUndefined: true }),
            typeOfSetAsideDescription: fc.option(fc.string({ maxLength: 20 }), { nilWithUndefined: true }),
            placeOfPerformance: fc.option(
              fc.record({
                state: fc.option(
                  fc.record({ name: fc.option(fc.string({ maxLength: 50 }), { nilWithUndefined: true }) }),
            ...
            award: fc.option(
              fc.record({ amount: fc.option(fc.integer({ min: 0, max: 10000000 }), { nilWithUndefined: true }) }),
              { nilWithUndefined: true }
            ),
            description: fc.option(fc.string({ maxLength: 500 }), { nilWithUndefined: true }),
            postedDate: fc.option(fc.string({ maxLength: 30 }), { nilWithUndefined: true }),
            responseDeadLine: fc.option(fc.string({ maxLength: 30 }), { nilWithUndefined: true }),
          }),
          { minLength: 0, maxLength: 20 }
        ),
        fc.option(fc.uuid(), { nilWithUndefined: true }),
        (opportunities, sourceId) => {
          // Run upsert twice with the same batch
          const records1 = new Map<string, ReturnType<typeof mapOpportunityToRecord>>()
          const result1 = simulateUpsertBatch(records1, opportunities, sourceId)

          const records2 = new Map<string, ReturnType<typeof mapOpportunityToRecord>>()
          // First batch
          simulateUpsertBatch(records2, opportunities, sourceId)
          // Second batch (same data)
          const result2 = simulateUpsertBatch(records2, opportunities, sourceId)
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          errorMessage: fc.option(fc.string(), { nilWithUndefined: true }),
        }),
    ...
    fc.assert(
      fc.property(
        ...
        (opp, sourceId) => {
          ...
          expect(record.sam_or_source_url).toMatch(/^https:\/\/sam\.gov\/opp\//)
          ...
          expect(record.place_of_performance).toBe(opp.placeOfPerformance?.state?.name ?? null)
      ...
      fc.property(
        fc.record({
          companies: fc.array(
            fc.record({
              id: fc.uuid(),
              naics_codes: fc.array(fc.string({ maxLength: 10 }), { maxLength: 5 }),
              socio_economic_certs: fc.array(fc.string({ maxLength: 20 }), { maxLength: 5 }),
              target_geographies: fc.array(fc.string({ maxLength: 50 }), { maxLength: 5 }),
              capability_keywords: fc.array(fc.string({ maxLength: 30 }), { maxLength: 10 }),
            }),
          ...
          opportunities: fc.array(
            fc.record({
              id: fc.uuid(),
              naics_code: fc.option(fc.string({ maxLength: 10 }), { nilWithUndefined: true }),
              set_aside: fc.option(fc.string({ maxLength: 20 }), { nilWithUndefined: true }),
              place_of_performance: fc.option(fc.string({ maxLength: 50 }), { nilWithUndefined: true }),
              value_max: fc.option(fc.integer({ min: 0, max: 10000000 }), { nilWithUndefined: true }),
              value_min: fc.option(fc.integer({ min: 0, max: 10000000 }), { nilWithUndefined: true }),
        ...
        ({ companies, opportunities }) => {
          ...
          const scores: { company_id: string; opportunity_id: string; score: number }[] = []
